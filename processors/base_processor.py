...
class BaseProcessor:
    ...
    def maskOutside(self, image, aoi):
        """
    ...
    def start_export(self, image, scale, description, region, filename_prefix, crs):
        """
        ...
        logger.info("Exporting  with Task ID: {} file {} ...".format(task_id, filename_prefix))
    ...
    def check_product_update(self, product_name, date_string):
        ...
        """
        if settings.force_processing:
            logger.warning('Processing is being forced')
            return True

        target_date = datetime.datetime.strptime(date_string, "%Y-%m-%d").date()

        with open(settings.last_product_updates, "r", newline="", encoding="utf-8") as f:
    ...
    def update_product_status_file(self, input_dict, output_file):
        """
    ...
    def get_github_info(self):
        ...
        owner = settings.config['GITHUB_OWNER']
        repo = settings.config['GITHUB_REPO']
    ...
    def prepare_export(self, roi, productitem, productasset, productname, scale, image, sensor_stats, current_date_str):
        """
        ...
        processor_version = self.get_github_info()

        # Define the quadrants to split into 4 regions
        quadrants = self.get_quadrants(roi)
        ...
        for quadrant_name, quadrant in quadrants.items():
            ...
            self.start_export(image, int(scale),
                         "P:" + productname + " I:" + productasset, quadrant, filename_q, settings.OUTPUT_CRS)
        ...
        self.update_product_status_file(product_status, settings.last_product_updates)
        ...
        data = [productname, productitem, productasset, str(sensor_stats[0]), str(
            sensor_stats[1]), str(sensor_stats[2]), current_date_str, processor_version["GithubLink"],
                processor_version["ReleaseVersion"]]

        with open(os.path.join(settings.processing_dir, productasset + ".csv"), "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(header)
            writer.writerow(data)
